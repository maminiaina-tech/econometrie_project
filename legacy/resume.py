...
import pdfplumber, docx, os, re, textwrap, threading
...
def choose_pipe(txt: str):
    return (summ_small, tok_small, 1024) if len(tok_small.encode(txt)) <= 1024 else (summ_large, tok_large, 4096)
...
slider  = tk.Scale(root, from_=50, to=800, variable=length_var, orient="horizontal", length=200, resolution=10, tickinterval=150, bg=COL_BG, highlightthickness=0)
